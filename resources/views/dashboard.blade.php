                    ...
                    @if($user->id !== auth()->user()->id)
                        @if( !$user->siguiendo( auth()->user() ) )
                            <form action="{{ route('users.follow', $user) }}" method="POST">
                                ...
                                <input 
                                    type="submit"
                                    class="bg-blue-600 text-white uppercase rounded-lg px-3 py-1 text-xs font-bold cursor-pointer shadow"
                                    value="Seguir"
                                    />
                            </form>
                        @else
                            <form action="{{ route('users.unfollow', $user) }}" method="POST">
                                ...
                                <input 
                                    type="submit"
                                    class="bg-white text-blue-600 uppercase rounded-lg px-3 py-1 text-xs font-bold cursor-pointer shadow"
